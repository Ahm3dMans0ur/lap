<div class="form-group xxs-pt-40 clearfix{{ $errors->has('specs.'.$field['id']) ? ' has-error' : '' }}">
  ...
  <div class="col-xs-10 col-sm-7">
    @if ($errors->has('specs.'.$field['id'])) <p class="help-block">{{ $errors->first('specs.'.$field['id']) }}</p> @endif
        {!! Form::text("specs[{$field['id']}]", (isset($field['pivot']) ? $field['pivot']['value'] : null), ['class' => (($field['key']=='color')? 'jscolor':'').' form-control font-droidkufi xxs-pt-10 xxs-pb-30 xxs-pr-0 text-right lang-ltr','placeholder'=>$field['default']]) !!}
