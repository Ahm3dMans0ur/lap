{{-- todo : merge this file with main categories file
    fix main categorey link issuse
 --}}
        ...
        <div class="xxs-mt-20 xxs-mb-20 z_side-cat-childs">
            @if($mainCategory['childs'])
                @foreach($mainCategory['childs'] as $chiled_category)
                    <a href="{{route('categories.services.show',$chiled_category['slug'])}}"
                       class="display-block font-jfflat xxs-mb-15 clearfix text-primary @if(isset($category) && $category['id'] == $chiled_category['id']) text-muted @endif">
                        @if($chiled_category->services)<span
                                class="square-sm pull-right text-light bg-gray xxs-mr-20 xxs-mt-2 border-radius-round text-sm text-center line-height-lg"> {{$chiled_category->services->count()}} </span>@endif
